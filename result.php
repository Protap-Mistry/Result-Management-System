			...
			if(isset($_POST["submit"]))
			{
				...
				if($object->row_count() > 0)
				{
					foreach($object->statement_result() as $student_data)
					{
						...
						<p><b>Email Address: </b>'.$student_data["student_email_id"].'</p>
						<div class="row">
							<div class="col-md-6">
					...
					foreach($exam_result as $exam_data)
					{
						...
						<div class="row">
							<div class="col-md-6">
								<p><b>Exam Type: </b>'.$exam_data["exam_name"].'</p>
							</div>
							<div class="col-md-6">
						...
						if($result["result_status"] == "Enable")
						{
							...
							foreach($marks_data as $marks)
							{
								...
								echo '
									<tr>
										<td>'.$count.'</td>
										<td class="table-secondary">'.$marks["subject_name"].'</td>
										<td>'.$marks["marks"].'</td>
									</tr>
								';
								$total += $marks["marks"];
							...
							';
							$download_button = '<a href="download.php?exam_id='.$_POST['exam_name'].'&student_roll_no='.$_POST["student_roll_no"].'" class="btn btn-success"><i class="fas fa-file-pdf-o" aria-hidden="true"></i> Download The Result</a>';
						}
						else
						{
